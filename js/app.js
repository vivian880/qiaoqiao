...
(function () {
  ...
  function renderQuiz() {
    ...
    let html = `
      ...
      ${item.reading
        ...
        : (item.char
            ...
            : (() => {
                const isFill = !item.char && /（\s*）/.test(item.text || '')
                const cls = q.key === 'rifle' ? 'q-text-math' : (isFill ? 'q-text-fill' : 'q-text')
                return `<div class="${cls}">${rubyfy(esc(item.text)).replace('（ ）', '<span class="blank">（ ）</span>')}</div>`
              })())}
      <div class="options">`
    ...
    item.options.forEach((o, i) => {
      ...
      html += `<div class="${cls}" data-action="answer" data-i="${i}"><span class="opt-label">${label}</span>${rubyfy(esc(o))}</div>`
    })
    html += `</div>`
    if (q.answered) {
